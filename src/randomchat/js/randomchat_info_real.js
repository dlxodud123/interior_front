...
const Randomchat_info = () => {
    ...
    const [isOtherConnected, setIsOtherConnected] = useState(false); // 상대방 접속 상태
    ...
    useEffect(() => {
        ...
        newSocket.on('userConnected', (data) => {
            console.log('Other user connected:', data.socketId);
            setIsOtherConnected(true);
        });
        // 상대방 접속 종료 이벤트 처리
        newSocket.on('userDisconnected', (data) => {
            console.log('Other user disconnected:', data.socketId);
            setIsOtherConnected(false);
        });
                        ...
                        <div className='randomchat_info_text_container'>
                            ...
                            <div className='randomchat_info_text_content'>
                                {messages.map((msg, index) => (
                                    <div className={msg.sender === "me" ? 'randomchat_info_text_me' : 'randomchat_info_text_other'}>
